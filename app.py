...
@keyframes sidebarGlow {
    from { box-shadow: 5px 0 30px rgba(102, 126, 234, 0.2); }
    to { box-shadow: 5px 0 40px rgba(102, 126, 234, 0.4); }
}
...
[data-testid="stSidebar"] button {
    ...
    animation: slideInBounce 0.8s ease-out forwards !important;
}

[data-testid="stSidebar"] button:nth-child(1) { animation-delay: 0.1s !important; }
[data-testid="stSidebar"] button:nth-child(2) { animation-delay: 0.2s !important; }
[data-testid="stSidebar"] button:nth-child(3) { animation-delay: 0.3s !important; }
[data-testid="stSidebar"] button:nth-child(4) { animation-delay: 0.4s !important; }
[data-testid="stSidebar"] button:nth-child(5) { animation-delay: 0.5s !important; }
[data-testid="stSidebar"] button:nth-child(6) { animation-delay: 0.6s !important; }
[data-testid="stSidebar"] button:nth-child(7) { animation-delay: 0.7s !important; }

@keyframes slideInBounce {
    0% { transform: translateX(-20px) scale(0.95); opacity: 0.7; }
    50% { transform: translateX(5px) scale(1.02); opacity: 0.9; }
...
def get_risk_alerts(data):
    ...
    if 'Timestamp' in data.columns:
        ...
        recent_intrusions = data[data['Is_Intrusion'] == 1].shape[0]
...
def main():
    # SIDEBAR NAVIGATION
    with st.sidebar:
        st.markdown('<h2 class="sidebar-header">🛡️ Navigation</h2>', unsafe_allow_html=True)
        ...
        for i, (page_name, page_key) in enumerate(pages.items()):
            # Use Streamlit button with enhanced CSS targeting
        ...
        if current_page_name:
            st.markdown(f'<div class="current-page-indicator">📍 {current_page_name[0]}</div>', unsafe_allow_html=True)
    ...
    # (Duplicate ALERTS SYSTEM removed from here since it safely runs further down)
    ...
    with st.expander("⚙️ Advanced Controls & Filters", expanded=False):
        ...
        with col1:
            ...
            data_source = st.radio("Data Source Selection", ["Sample Data", "Upload CSV"], horizontal=True, label_visibility="collapsed")
        ...
        with col3:
            ...
            export_format = st.selectbox("Export Format Selection", ["CSV", "JSON", "Excel"], label_visibility="collapsed")
    ...
    try:
        ...
        if len(data) == 0:
            ...
            if st.button("🔄 Quick Regenerate Data", use_container_width=True, type="primary"):
                ...
                train_ai_model.clear()
                get_analytics_data.clear()
                get_hotspot_stats.clear()
                create_performance_metrics.clear()
                st.rerun()
            return
    ...
    if st.session_state.page == "dashboard":
        ...
        st.markdown('<h3 class="section-header">🗺️ Global Risk Hotspot Map & Tactical Overview</h3>', unsafe_allow_html=True)

        # Map customization options
        col1, col2, col3, col4 = st.columns(4)
        with col1:
            map_view = st.selectbox("Map Mode", ["Tactical Command", "Standard Risk Map", "Glowing Density Map", "24H Threat Time-Lapse"], index=0)
        with col2:
        ...
        # HQ location for tactical map (Geneva)
        hq_lat, hq_lon = 46.2044, 6.1432
        ...
        if map_view == "Glowing Density Map":
            ...
            fig_map = px.density_mapbox(
                density_data, lat="Latitude", lon="Longitude", z="Weight", radius=35,
                center=dict(lat=30, lon=20), zoom=2, height=750,
                hover_name="Hotspot", hover_data=['Country', 'Risk_Score'],
                title="🔥 Military Thermal Density Map"
            )
            fig_map.update_traces(coloraxis="coloraxis")
            fig_map.update_layout(coloraxis_colorscale="inferno")
            
        elif map_view == "24H Threat Time-Lapse":
            ...
            fig_map = px.scatter_mapbox(
                ...
                color_continuous_scale="RdYlGn_r", size_max=45, zoom=2, height=750,
        ...
        elif map_view == "Tactical Command":
            # Tactical Map with connections to HQ and glow effects
            
            # Draw connection lines from HQ to Hotspots
            for _, row in map_df.iterrows():
                line_color = "rgba(255, 68, 68, 0.4)" if row['Risk_Score'] > 0.5 else "rgba(102, 126, 234, 0.3)"
                fig_map.add_trace(go.Scattermapbox(
                    mode="lines",
                    lon=[hq_lon, row['Longitude']],
                    lat=[hq_lat, row['Latitude']],
            ...
            fig_map.add_trace(go.Scattermapbox(
                mode="markers+text",
                lon=[hq_lon], lat=[hq_lat],
                marker={"size": 15, "color": "#f093fb", "symbol": "circle"},
                text=["🌐 GLOBAL HQ"],
                textposition="bottom right",
                textfont={"color": "white", "size": 14, "family": "Courier New"},
                hoverinfo="text",
                hovertext="Geneva Intelligence Command Center",
                name="Headquarters"
            ))
            ...
            fig_map.add_trace(go.Scattermapbox(
                mode="markers",
                lon=map_df['Longitude'], lat=map_df['Latitude'],
                marker={
                    "size": map_df[size_metric] if size_metric == 'Breach_Count' else (map_df['Risk_Score']*80).clip(upper=100),
                    "sizemode": 'area', "sizeref": 2.*max(map_df[size_metric])/(80.**2) if size_metric == 'Breach_Count' else 1, "sizemin": 15,
                    "color": map_df[color_metric],
                    "colorscale": "RdYlGn_r",
                    "opacity": 0.3,
                    "showscale": False
                },
            ...
            # Hotspots Core Marker
            fig_map.add_trace(go.Scattermapbox(
                mode="markers",
                lon=map_df['Longitude'], lat=map_df['Latitude'],
                marker={
                    "size": map_df[size_metric] if size_metric == 'Breach_Count' else (map_df['Risk_Score']*30).clip(upper=40),
                    "sizemode": 'area', "sizeref": 2.*max(map_df[size_metric])/(40.**2) if size_metric == 'Breach_Count' else 1, "sizemin": 8,
                    "color": map_df[color_metric],
                    "colorscale": "RdYlGn_r",
                    "colorbar": {"title": color_metric, "x": 0.98},
                    "showscale": True
                },
                text=map_df['Hotspot'] + "<br>Risk: " + map_df['Risk_Pct'].round(1).astype(str) + "%<br>Breaches: " + map_df['Breach_Count'].astype(str),
                hoverinfo="text",
                name="Hotspots"
            ))
            
            fig_map.update_layout(
                title="📡 Tactical Command Control | Uplink Active",
                height=750,
                mapbox_pitch=45,
                mapbox_bearing=15,
                mapbox_center=dict(lat=35, lon=15),
                mapbox_zoom=1.8
            )

        else:
            # Standard Map View (enhanced sizes and colors)
            fig_map = px.scatter_mapbox(
                map_df, lat="Latitude", lon="Longitude", size=size_metric, color=color_metric,
                hover_name="Hotspot", hover_data={'Country': True, 'Risk_Pct': ':.1f', 'Breach_Count': True, 'Latitude': False, 'Longitude': False},
                color_continuous_scale="Inferno", size_max=50, zoom=2, height=750,
                center=dict(lat=30, lon=20),
                title="🔴 High Risk Zones | Standard Intelligence",
        ...
        fig_map.update_layout(
            ...
            paper_bgcolor='rgba(0,0,0,0)',
            font_color='white',
            title_font={"size": 24, "color": "#a0a0c0"}
        )
        ...
        with col1:
            ...
            new_font_size = st.slider("Font Size", 12, 24, st.session_state.font_size)
